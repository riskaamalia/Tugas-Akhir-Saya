...
for($a = 0; $a < sizeof($token_max_key); $a++){
	$id_term = NULL;
	//select id_term from term where term = $token_max_key[$a]
	//menggunakan thrift
...
if($status == 2){
		...
		for ($y=0;$y<1114;$y++) {	//jumlah id_url
				...
				$obj_query = new teshbase ();
				$penampung_idurl = $y;															//database
				$penampung_idterm = $id_term;
				$penampung_bobot = $obj_query -> select("url",$y,"url_pembobotan_web:".$id_term."=BOBOT");
				if ($penampung_bobot != NULL) {
				unset($temp);
				$temp[0][$index_nilai] = $y;	//id_url
				$temp[1][$index_nilai] = $id_term;	//id_term
